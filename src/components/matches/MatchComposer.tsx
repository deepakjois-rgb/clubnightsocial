...
type MatchComposerMode = "queue" | "court";

type MatchComposerProps = {
  players: Player[];
  mode: MatchComposerMode;
  courtName?: string;
...
export function MatchComposer({
  ...
  function buildPayload(): CreateQueuedMatchPayload | null {
  ...
  function handleStartNow() {
    const payload = buildPayload();
    if (!payload || !onStartNow) return;
    onStartNow(payload);
  }

  const allSelectedWaiting =
    selectedPlayerIds.length === requiredCount &&
    selectedPlayerIds.every(
      (id) => players.find((p) => p.id === id)?.state === "WAITING"
    );

  const startOnCourtLabel = courtName
    ? `${M.LIVE_START_ON_COURT} ${courtName}`
    : M.LIVE_START_MATCH;
      ...
      {mode === "court" ? (
        <div className="space-y-2">
          <Button
            variant="primary"
            fullWidth
            disabled={!allSelectedWaiting}
            onClick={handleStartNow}
          >
            {startOnCourtLabel}
          </Button>
          <Button variant="secondary" fullWidth onClick={handleAddToQueue}>
        ...
        <div className="flex gap-2">
          <Button variant="primary" fullWidth onClick={handleAddToQueue}>
            {M.QUEUE_ADD_TO_QUEUE}
          </Button>
          {onCancel && (
